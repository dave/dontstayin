...
using jQueryApi;
using Js.Library;
//using Js.Controls.ThreadControl;
...
namespace Js.Controls.CommentsDisplay
{
	...
	{
		private View view;
		public ThreadCommentsProvider threadCommentsProvider;
		...
		private int commentsPerPage { get { return int.Parse(view.uiCommentsPerPage.Value); } }
		...
		public EventHandler OnCommentsDisplayed;
		...
		private static void removeAllChildren(Element domElement)
		{
		...
		private Element createAnchor(CommentStub comment)
		{
	...
	<a href="<%#CurrentComment.Usr.Url()%>"><%#CurrentComment.Usr.NickName%></a>
		...
		*/
		private DivElement createCommentLeft(CommentStub comment)
		{
			...
			createMouseOverAndOut(jQuery.FromElement(aimg), comment.usrRollover, "htm();");
...
<div class="CommentBody">
	...
	<div class="CommentAdmin">
		...
		*/
		private DivElement createCommentBody(CommentStub comment)
		{
			...
			if (comment.haveAlreadyLold == false && bool.Parse(view.uiUsrIsLoggedIn.Value))
			{
				...
				AnchorElement lolAnchor = (AnchorElement)Document.CreateElement("a");
				{
					...
					jQuery.FromElement(lolAnchor).Click(lolClick);
				}
			...
			quote.Href = "#";
			jQuery.FromElement(quote)
				.MouseDown(quoteMouseDown)
				.Click(quoteClick);
			...
			if (comment.deleteLinkVisible)
			{
				...
				deleteAnchor.Href = "#";
				jQuery.FromElement(deleteAnchor).Click(deleteClick);
			...
			createMouseOverAndOut(jQuery.FromElement(postDetails), "stt('" + comment.k + "');", "htm();");
		...
		private void createMouseOverAndOut(jQueryObject element, string mouseover, string mouseout)
		{
			element
				.RemoveAttr("onmouseover")
				.RemoveAttr("onmouseout")
				.MouseOver(delegate(jQueryEvent e) { Script.Eval(mouseover); })
				.MouseOut(delegate(jQueryEvent e) { Script.Eval(mouseout); });
		...
		#endregion
		#region Lol
		private void lolClick(jQueryEvent e)
		{
			e.PreventDefault();
	...
	{
		public readonly static string DeleteConfirmText = "ConfirmText";
		public readonly static string CommentK = "CommentK";
		public readonly static string UsrK = "UsrK";
	}
}
