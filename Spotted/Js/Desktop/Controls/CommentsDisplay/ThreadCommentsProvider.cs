...
{
	public class ThreadCommentsProvider
	{
		int commentsPerPage;
		int threadK;
		public int ThreadK
		...
		public void appendComment(CommentStub newComment)
		{
		...
		public void appendComments(CommentStub[] newComments)
		{
		...
		public void LoadThreadComments()
		{
			if (this.threadK > 0)
			{
				if (Comments == null)
		...
		public void ReloadThreadComments(int threadK, int pageNumber)
		{
		...
		public string CommentsAnchorName;
		void prevPageClick(jQueryEvent e)
		{
			e.PreventDefault();
			this.RefreshBanners();
			this.pageNumber--;
		...
		void nextPageClick(jQueryEvent e)
		{
			e.PreventDefault();
			this.RefreshBanners();
			this.pageNumber++;
		...
		public DivElement CreatePrevNextPaging()
		{
			...
			{
				prevPage.Href = "#";
				jQuery.FromElement(prevPage).Click(prevPageClick);
			}
			else
			{
			...
			{
				nextPage.Href = "#";
				jQuery.FromElement(nextPage).Click(nextPageClick);
			}
			else
			{
		...
		public DivElement CreateNumberedPaging()
		{
			...
			for (int i = 1; i <= LastPage; i++)
			{
				if (renderPageNumber[i])
				{
					...
					else
					{
						...
						jQuery.FromElement(uiPageNumber).Click(pageClick);
		...
		public void CreatePublicThread(int currentParentObjectType, int currentParentObjectK, string duplicateGuid, string rawCommentHtml,
		...
		public void CreateNewPublicThread(int currentParentObjectType, int currentParentObjectK, string duplicateGuid, string rawCommentHtml,
			bool formatting, bool isNews, string[] inviteUsrOptions)
		{
		...
		public void CreateReply(int currentParentObjectType, int currentParentObjectK, int currentThreadK, string duplicateGuid, string rawCommentHtml,
		...
		public void DecrementCurrentThreadTotalComments()
		{
			this.TotalComments--;
		}
	}
