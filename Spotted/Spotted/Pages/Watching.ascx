<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="Watching.ascx.cs" Inherits="Spotted.Pages.Watching" %>

<asp:Panel Runat="server" ID="PanelWatching">
	<h1 class="TabHolder">
		...
		<a href="/pages/watching" class="TabbedHeading Selected" onclick="try { return WhenLoggedInAnchor(this); } catch(ex) { return false; }">Watching<!--<img src="/gfx/icon-eye-up.png" class="TabbedHeadingIcon" border="0" height="21" width="26">--></a>
	...
	<div class="ContentBorder">
		<p>
			This is your archive. It contains all the topics you're watching (even 
			if they don't have new messages).
		</p>
	</div>
	<asp:Panel Runat="server" ID="NoThreadsPanel" EnableViewState="False">
		...
		<div class="ContentBorder">
			<p align="center">
				You're not watching any topics.
			</p>
			<p align="center">
				...
				<asp:DataGrid Runat="server" ID="ThreadsDataGrid" GridLines="None" AutoGenerateColumns="False" BorderWidth=0 CellPadding=3 CssClass=dataGrid OnItemDataBound="ThreadsDataGrid_ItemDataBound"
					...
					<Columns>
						...
						<asp:TemplateColumn HeaderText="" ItemStyle-CssClass="dataGridThreadTitles" ItemStyle-Width="100%">
							<ItemTemplate>
								<%#((Bobs.Thread)Container.DataItem).IconsHtml(ContainerPage.Url)%><%#((Bobs.Thread)Container.DataItem).CommentHtmlStart%><a href="<%#Bobs.UrlInfo.MakeUrl("","chat","a",ThreadPage.ToString(),"k",((Bobs.Thread)Container.DataItem).K.ToString())%>" <%#((Bobs.Thread)Container.DataItem).Rollover%>><%#HttpUtility.HtmlEncode(((Bobs.Thread)Container.DataItem).SubjectSafe)%></a><%#((Bobs.Thread)Container.DataItem).CommentHtmlEnd%><%#((Bobs.Thread)Container.DataItem).PagesHtml(ContainerPage.Url, new Bobs.Thread.GetUrlDelegate(GetThreadUrl),"a",ThreadPage.ToString(),"k",((Bobs.Thread)Container.DataItem).K.ToString())%>
							</ItemTemplate>
						</asp:TemplateColumn>
