...
using Sys.UI;
using Utils;
using Spotted.WebServices.Controls.Banners.Generator;
namespace SpottedScript.Controls.Banners.Generator
{
	public class Controller
	{
		...
		{
			this.view = view;
			this.pos = int.ParseInvariant(this.view.uiPosition.Value);
			this.timeout = int.ParseInvariant(this.view.uiDuration.Value);
			if (this.view.uiK.Value.Length > 0)
			{
				this.lastK = int.ParseInvariant(this.view.uiK.Value);
				this.StartTimer();
			}
			...
			userInactivityPeriodDuration = int.ParseInvariant(this.view.uiInactivityTimeoutDuration.Value);
		...
		private void OnUserAction(DomEvent e)
		{
		...
		private bool BannerIsVisible()
		{
			...
			JQueryObject j = JQueryAPI.JQuery(this.view.uiBanner);
...
//#endif
							div.Style.Left = (0 + int.ParseInvariant(this.view.uiClickHelperLeft.Value) ) + "px";
							div.Style.Top = (0 + int.ParseInvariant(this.view.uiClickHelperLeft.Value) ) + "px";
					...
					DomEvent.AddHandler(this.view.uiBanner, "mouseover", StopTimer);
				}
			}
			else
			{
		...
		private void StopTimer(DomEvent e)
		{
