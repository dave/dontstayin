	...
	public class Controller
	{
		...
		private int objectK
		{
			get
			{
				try
				{
					return int.ParseInvariant(view.uiObjectK.Value);
				}
				catch
				{
					return 0;
		...
		{
			this.view = view;
			latestThreadsProvider = new LatestThreadsProvider(int.ParseInvariant(view.uiThreadsCount.Value), bool.Parse(view.uiHasGroupObjectFilter.Value), int.ParseInvariant(view.uiObjectType.Value));
		...
		internal void Show(int objectK)
		{
