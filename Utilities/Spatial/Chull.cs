...
namespace Microsoft.Samples.SqlServer {
	...
	public class Chull : HtmShape , IComparer {
		...
		//   Input:  input[] = an array of 2D points 
		//       presorted by increasing x- and y-coordinates
		//     n =  the number of points in input[]
		//   Output: output[] = an array of the convex hull vertices  (max is n)
		//  Return: the number of points in output[]
		
		int chainHull_2D( ) { // CPoint* P, int n, CPoint* H )
			// the output array H[] will be used as the stack
		
			// int *S = this->indexStack;
			...
			int minmin = 0, minmax;
			int n = input.Count;
			double xmin = ((CPoint) input[0]).x;
			
			for(i=1; i<n; i++) { // This loop sets i for minmax below
				if((((CPoint) input[i])).x != xmin)
					break;
			}
			...
			if (minmax == n-1) { //  degenerate case: all x-coords == xmin
				...
				//was: output[++top] = input[minmin];
				top++;
				output.Add(input[minmin]);
				// S[top] = minmin;
				((CPoint) input[minmin]).keeper = true;
		
				if (((CPoint) input[minmax]).y != ((CPoint) input[minmin]).y) {// a  nontrivial segment
					//was: output[++top] =  input[minmax];
					top++;
					output.Add(input[minmax]);
					// S[top] = minmax;
					((CPoint)input[minmax]).keeper = true;
				}
				//was: output[++top] = input[minmin];	// add polygon endpoint
				top++;
				output.Add(input[minmin]);
				// S[top] = minmin;
				((CPoint) input[minmin]).keeper = true;
			...
			double xmax = ((CPoint) input[n-1]).x;
			for (i=n-2; i>=0; i--)
				if (((CPoint) input[i]).x != xmax)
					break;
			maxmin = i+1;
		 
			// Compute the lower hull on the stack H
			// was: output[++top] = input[minmin];		// push  minmin point onto stack
			top++;
			output.Add(input[minmin]);
			// S[top] = minmin;
			((CPoint)input[minmin]).keeper = true;
			...
			while (++i <= maxmin){
				...
				if (isLeft(
					(CPoint) input[minmin],
					(CPoint) input[maxmin], 
					(CPoint) input[i]) >= 0 && i < maxmin)
					continue;  // ignore P[i] above or on the lower line
				while (top > 0) {// there are at least 2 points on the stack
					// test if  P[i] is left of the line at the stack top
					if (isLeft(  output[top-1], output[top], input[i]) > 0)
				...
				//was: output[++top] = input[i];  // push P[i] onto stack
				top++;
				output.Add(input[i]);
				// S[top] = i;
				((CPoint) input[i]).keeper = true;
			}
		 
			// Next, compute the upper hull on the stack H above  the bottom hull
			if (maxmax != maxmin){  // if  distinct xmax points
				// was: output[++top] = input[maxmax]; // push maxmax point onto stack
				top++;
				output.Add(input[maxmax]);
				// S[top] = maxmax;
				((CPoint) input[maxmax]).keeper = true;
			}
			...
			if (minmax != minmin){
				// was: output[++top] = input[minmin];	// push joining endpoint onto stack
				top++;
				output.Add(input[i]);
				// S[top] = minmin;
				((CPoint) input[minmin]).keeper = true;
...
//  if (plate.xform()){ 
//    cerr << "plate is more than a hemisphere" << endl;
//  } else {
